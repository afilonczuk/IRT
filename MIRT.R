...
############## Begin Simulation ########################
#Note: if you change the number of items, you will have to add/remove coefficients in line 78 
...
new<- try(mirt(out, model = 2, pars = sv, itemtype = '2PL'))
  ...
  if (inherits(new, "try-error")) {
    count = count+1
        ...
        #(note:this code is written for 10 items and must be revised if the number of items is changed)
...
rmse <- colSums(sqrt(sqbias/trials))/n
rmse
#the average bias for each parameter per item
write.table(round(dif/(trials), 4))
#the average bias for each parameter averaged over the items
write.table(round(colSums(dif/(trials))/n, 4))
